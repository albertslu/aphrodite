...
class ImageCleaner:
    def __init__(self, image_dir, removed_images_dir):
        ...
        # Define text prompts focusing on person with dog
        self.text_prompts = [
            "a photo of a person with a dog",
            "a person posing with their pet dog",
            "a human and dog together",
            "a photo without any people",
            "just a dog without people",
            "an empty room",
            "just food or drinks",
    ...
    def analyze_image(self, image_path):
        try:
            ...
            # Extract scores
            person_with_dog_score = max(similarity[0][0:3].tolist())  # First three prompts are positive
            negative_signals = max(similarity[0][3:].tolist())  # Last five prompts are negative
            
            # Keep image if it has clear person with dog presence and low negative signals
            if person_with_dog_score > 0.3 and negative_signals < 0.5:
                return True
            else:
                print(f"Image {image_path} does not meet criteria (person_with_dog: {person_with_dog_score:.2f}, negative: {negative_signals:.2f})")
...
def main():
    parser = argparse.ArgumentParser(description='Clean person with dog photos dataset')
    parser.add_argument('--image_dir', default='dating_app_dataset/person_with_dog', help='Directory containing the images')
